...
2. Weekly full refresh  
   在复权模式（`qfq/hfq`）下，到每周触发日（默认周日）时，CLI 询问是否执行近似全量回补；确认后使用回补天数 `99999`。
...
- 如需调整，请修改 `sync_a_share_to_sqlite.py` 顶部常量区（统一管理）：
  `TOKEN_FILE_PATH`、`REQUEST_SLEEP_SECONDS`、`REQUEST_MAX_RETRIES`、`REQUEST_RETRY_BACKOFF_SECONDS`、`DAILY_ADJUST_BACKFILL_DAYS`、`WEEKLY_FULL_WEEKDAY`、`WEEKLY_FULL_BACKFILL_DAYS`。
...
1. `--ts-token`
2. `TS_TOKEN` 环境变量
3. 脚本固定 token 文件路径 `data/ts_token.txt`
